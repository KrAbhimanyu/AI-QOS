...
def render_chat_panel() -> None:
    ...
    st.markdown(f"""<div style=" background: rgba(30, 30, 63, 0.8); border: 1px solid rgba(99, 102, 241, 0.2); border-radius: 12px; padding: 1rem; margin-bottom: 1rem; display: flex; align-items: center; justify-content: space-between; "> <div> <h3 style="color: #F1F5F9; margin: 0; font-size: 1rem;">💬 Login Flow Analysis</h3> <p style="color: #64748B; margin: 0.25rem 0 0; font-size: 0.8rem;">3 messages • Active now</p> </div> <div style="display: flex; gap: 0.5rem;"> <span style="color: #64748B; cursor: pointer;">📌</span> <span style="color: #64748B; cursor: pointer;">⋮</span> </div> </div>""", unsafe_allow_html=True)

    # Scrollable chat area
    st.markdown("""<div style=" height: 400px; overflow-y: auto; padding: 1rem; background: rgba(15, 15, 26, 0.5); border-radius: 12px; margin-bottom: 1rem; ">""", unsafe_allow_html=True)
    ...
    for i, card in enumerate(context_cards):
        with cols[i]:
            st.markdown(f"""<div style=" text-align: center; padding: 0.75rem 0.5rem; background: rgba(30, 30, 63, 0.8); border: 1px solid rgba(99, 102, 241, 0.2); border-radius: 8px; cursor: pointer; "> <span style="font-size: 1.25rem;">{card['icon']}</span> <p style="color: #F1F5F9; margin: 0.25rem 0 0; font-size: 0.7rem;">{card['name']}</p> <p style="color: #64748B; margin: 0; font-size: 0.65rem;">{card['desc']}</p> </div>""", unsafe_allow_html=True)
