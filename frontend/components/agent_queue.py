...
def render_queue_visual(agents: list[dict[str, Any]]) -> None:
    ...
    for agent in agents:
        ...
        st.markdown(f'''
        <div class="queue-dot" 
             style="background: {config['bg_color']}; border: 2px solid {config['color']}50;"
             title="{agent['name']}: {agent['status'].value}">
            {agent['icon']}
        </div>
        ''', unsafe_allow_html=True)
...
def agent_queue(agents: list[dict[str, Any]]) -> None:
    ...
    with col2:
        ...
        <div style="
            padding: 16px;
            background: linear-gradient(135deg, {failed_color}20, {failed_color}10);
            border: 1px solid {failed_color}30;
            border-radius: 12px;
            text-align: center;
        ">
            <div style="font-size: 36px; font-weight: 700; color: {failed_color};">
                {failed}
            </div>
            <div style="font-size: 12px; color: #64748b; text-transform: uppercase;">
                Failed Agents
