        ...
        ">
            <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 1rem;">
                <div>
                    ...
                    <p style="margin: 0.5rem 0 0; color: #94A3B8; font-size: 0.875rem;">{description}</p>
                </div>
                <span style="
                    padding: 0.25rem 0.75rem;
                    border-radius: 9999px;
                    ...
                    background: {color}20;
                    color: {color};
                ">{status.capitalize()}</span>
            </div>
            <div style="display: flex; gap: 1.5rem; color: #94A3B8; font-size: 0.875rem;">
                <span>🤖 {agent}</span>
                <span>📅 {format_timestamp(created_at)}</span>
            </div>
            {f'<div style="margin-top: 1rem; height: 4px; background: #334155; border-radius: 2px;"><div style="width: {progress}%; height: 100%; background: {color}; border-radius: 2px;"></div></div>' if progress is not None else ''}
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def agent_card(
    ...
    st.markdown(
        f"""
        <div style="
            background: linear-gradient(135deg, #1E1E3F 0%, rgba(99, 102, 241, 0.05) 100%);
            border: 1px solid #334155;
            border-radius: 12px;
            padding: 1.5rem;
            margin-bottom: 1rem;
        ">
            <div style="display: flex; align-items: center; gap: 1rem; margin-bottom: 1rem;">
                ...
                <div>
                    ...
                    <p style="margin: 0; color: #94A3B8; font-size: 0.875rem;">{role}</p>
                </div>
            </div>
            <div style="display: flex; justify-content: space-between; align-items: center;">
                ...
                <div style="text-align: right;">
                    <p style="margin: 0; color: #F1F5F9; font-weight: 500;">{tasks_completed} tasks</p>
                    <p style="margin: 0; color: #94A3B8; font-size: 0.75rem;">{success_rate}% success</p>
                </div>
            </div>
        </div>
        """,
...
def execution_card(
    ...
    st.markdown(
        ...
        ">
            <div>
                <p style="margin: 0; color: #F1F5F9; font-weight: 500;">{mission_name}</p>
                <p style="margin: 0.25rem 0 0; color: #64748B; font-size: 0.75rem;">ID: {execution_id}</p>
            </div>
            <div style="text-align: right;">
                <span style="
                    padding: 0.25rem 0.75rem;
                    ...
                    color: {color};
                ">{status.capitalize()}</span>
                <p style="margin: 0.25rem 0 0; color: #64748B; font-size: 0.75rem;">{duration or format_timestamp(started_at)}</p>
            </div>
        </div>
        """,
        unsafe_allow_html=True,
