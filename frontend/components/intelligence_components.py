            ...
            <div style="display: flex; align-items: center; gap: 1rem; margin-bottom: 0.75rem;">
                ...
                <div style="flex: 1;">
                    <div style="display: flex; justify-content: space-between; margin-bottom: 0.25rem;">
                        <span style="color: #F1F5F9; font-weight: 500;">{phase_name}</span>
                        <span style="color: {color}; font-weight: 600;">{progress}%</span>
                    </div>
...
def phase_completed_badge(phase_name: str, icon: str = "✅") -> None:
    ...
    st.markdown(
        ...
        ">
            <span style="color: #10B981; font-size: 1.25rem;">{icon}</span>
            <span style="color: #94A3B8; font-size: 0.875rem;">{phase_name}</span>
            <span style="color: #10B981; margin-left: auto; font-size: 0.75rem;">Completed</span>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def glass_loading_panel(message: str = "Loading...") -> None:
    ...
    st.markdown(
        ...
        ">
            ...
            "></div>
            <p style="color: #F1F5F9; margin: 0;">{message}</p>
            <style>
                @keyframes spin {{
    ...
    st.markdown(
        ...
        ">
            <div style="display: flex; align-items: center; gap: 1rem; margin-bottom: 1.25rem;">
                ...
                ">🎯</div>
                <div>
                    <h3 style="color: #F1F5F9; margin: 0; font-size: 1.1rem;">{mission_name}</h3>
                    <p style="color: #94A3B8; margin: 0.25rem 0 0; font-size: 0.8rem;">{project}</p>
                </div>
            </div>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 0.75rem; margin-bottom: 1rem;">
                <div style="padding: 0.75rem; background: rgba(51, 65, 85, 0.5); border-radius: 8px;">
                    ...
                    <p style="color: #F1F5F9; margin: 0.25rem 0 0; font-size: 0.85rem; font-weight: 500;">{environment}</p>
                </div>
                <div style="padding: 0.75rem; background: rgba(51, 65, 85, 0.5); border-radius: 8px;">
                    <p style="color: #64748B; margin: 0; font-size: 0.7rem;">Priority</p>
                    <p style="color: {prio_color}; margin: 0.25rem 0 0; font-size: 0.85rem; font-weight: 500;">{priority}</p>
                </div>
                <div style="padding: 0.75rem; background: rgba(51, 65, 85, 0.5); border-radius: 8px;">
                    <p style="color: #64748B; margin: 0; font-size: 0.7rem;">Execution Mode</p>
                    <p style="color: #F1F5F9; margin: 0.25rem 0 0; font-size: 0.85rem; font-weight: 500;">{mode}</p>
                </div>
                <div style="padding: 0.75rem; background: rgba(51, 65, 85, 0.5); border-radius: 8px;">
                    <p style="color: #64748B; margin: 0; font-size: 0.7rem;">Started</p>
                    <p style="color: #F1F5F9; margin: 0.25rem 0 0; font-size: 0.85rem; font-weight: 500;">{started_at.strftime('%H:%M:%S')}</p>
                </div>
            </div>
            ...
            <div style="margin-bottom: 1rem;">
                <div style="display: flex; justify-content: space-between; margin-bottom: 0.5rem;">
                    ...
                    <span style="color: #6366F1; font-size: 0.8rem; font-weight: 600;">{progress}%</span>
                </div>
            ...
            <div style="display: flex; justify-content: space-between; align-items: center; padding-top: 1rem; border-top: 1px solid #334155;">
                <div>
                    <p style="color: #64748B; margin: 0; font-size: 0.7rem;">Current Phase</p>
                ...
                <span style="
                    ...
                    font-size: 0.75rem;
                    background: {stat_color}20;
                    color: {stat_color};
                ">
                    <span style="width: 8px; height: 8px; border-radius: 50%; background: {stat_color};"></span>
                    {status}
                </span>
            </div>
            
            <div style="display: flex; justify-content: space-between; margin-top: 1rem; padding-top: 1rem; border-top: 1px solid #334155;">
                <div>
                    <p style="color: #64748B; margin: 0; font-size: 0.7rem;">Elapsed</p>
                    <p style="color: #22D3EE; margin: 0.25rem 0 0; font-size: 1rem; font-weight: 600;">{elapsed}</p>
                </div>
                <div style="text-align: right;">
                    <p style="color: #64748B; margin: 0; font-size: 0.7rem;">Remaining</p>
                    <p style="color: #F59E0B; margin: 0.25rem 0 0; font-size: 1rem; font-weight: 600;">{remaining}</p>
                </div>
            </div>
        </div>
        """,
    ...
    st.markdown(
        ...
        ">
            <div style="display: flex; align-items: center; gap: 0.75rem; margin-bottom: 0.5rem;">
                <span style="font-size: 1.5rem;">{icon}</span>
                <div style="flex: 1;">
                    <p style="color: #64748B; margin: 0; font-size: 0.7rem;">{title}</p>
                    <p style="color: #F1F5F9; margin: 0; font-size: 0.95rem; font-weight: 500;">{value}</p>
                </div>
            </div>
            <div style="display: flex; justify-content: space-between; align-items: center;">
                {f'<span style="color: #94A3B8; font-size: 0.75rem;">v{version}</span>' if version else '<span></span>'}
                <span style="
                    display: inline-flex;
                    align-items: center;
                    gap: 0.25rem;
        ...
        ">
            <div style="display: flex; align-items: center; gap: 0.75rem; margin-bottom: 1rem;">
                <div style="
                    ...
                    font-size: 1.25rem;
                ">🤖</div>
                <h4 style="color: #F1F5F9; margin: 0;">AI Assistant</h4>
            </div>
            ...
            <div style="margin-bottom: 1rem;">
                <p style="color: #64748B; margin: 0 0 0.5rem; font-size: 0.75rem;">Current Activity</p>
                <p style="color: #22D3EE; margin: 0; font-size: 0.85rem;">{current_activity}</p>
            </div>
            
            <div style="margin-bottom: 1rem;">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.5rem;">
                    <p style="color: #64748B; margin: 0; font-size: 0.75rem;">Confidence Score</p>
            ...
            <div style="margin-bottom: 1rem;">
                <p style="color: #64748B; margin: 0 0 0.75rem; font-size: 0.75rem;">Interesting Findings</p>
                {''.join([f'<div style="padding: 0.5rem 0; border-bottom: 1px solid rgba(99, 102, 241, 0.1);"><span style="color: #10B981;">▸</span> <span style="color: #94A3B8; font-size: 0.8rem;">{finding}</span></div>' for finding in findings])}
            </div>
            ...
            <div style="
                ...
                margin-top: 1rem;
            ">
                <p style="color: #F59E0B; margin: 0 0 0.5rem; font-size: 0.75rem; font-weight: 500;">⚠️ Warning</p>
                <p style="color: #94A3B8; margin: 0; font-size: 0.8rem;">{warning}</p>
            </div>
...
def timeline_step(
    ...
    st.markdown(
        f"""
        <div style="display: flex; gap: 1rem; margin-bottom: {0 if is_last else 1.5}rem;">
            <div style="display: flex; flex-direction: column; align-items: center;">
                <div style="
                    ...
                    flex-shrink: 0;
                ">{icon}</div>
                {'' if is_last else f'<div style="width: 2px; flex: 1; min-height: 40px; background: {color}; margin-top: 0.5rem;"></div>'}
            </div>
            <div style="flex: 1; padding-bottom: {0 if is_last else 1}rem;">
                <p style="color: {'#F1F5F9' if status != 'pending' else '#64748B'}; margin: 0 0 0.25rem; font-weight: 500;">{step_name}</p>
                <p style="color: #64748B; margin: 0; font-size: 0.75rem;">{timestamp} {f'• {duration}' if duration else ''}</p>
            </div>
        </div>
        """,
        unsafe_allow_html=True,
...
def application_overview_card(
    ...
    st.markdown(
        ...
        <div style="
            ...
            margin-bottom: 1rem;
        ">
            <h3 style="color: #F1F5F9; margin: 0 0 1rem; font-size: 1.1rem;">Application Overview</h3>
            
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-bottom: 1rem;">
                <div style="padding: 1rem; background: rgba(51, 65, 85, 0.5); border-radius: 12px; text-align: center;">
                    <p style="color: #64748B; margin: 0 0 0.25rem; font-size: 0.7rem;">Website</p>
                    <p style="color: #F1F5F9; margin: 0; font-size: 1rem; font-weight: 600;">{website_name}</p>
                </div>
                <div style="padding: 1rem; background: rgba(51, 65, 85, 0.5); border-radius: 12px; text-align: center;">
                    <p style="color: #64748B; margin: 0 0 0.25rem; font-size: 0.7rem;">Technology</p>
                    <p style="color: #6366F1; margin: 0; font-size: 1rem; font-weight: 600;">{technology}</p>
                </div>
            </div>
            
            <div style="display: flex; justify-content: space-between; align-items: center; padding: 0.75rem; background: rgba(51, 65, 85, 0.5); border-radius: 8px; margin-bottom: 1rem;">
                <span style="color: #94A3B8;">🔐 Authentication</span>
                <span style="color: #10B981; font-weight: 500;">{authentication}</span>
            </div>
            
            <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 0.75rem;">
                {''.join([f'''
                <div style="text-align: center; padding: 0.75rem; background: rgba(51, 65, 85, 0.3); border-radius: 8px;">
                    <p style="color: #64748B; margin: 0 0 0.25rem; font-size: 0.65rem;">{label}</p>
                    <p style="color: #F1F5F9; margin: 0; font-size: 1.25rem; font-weight: 600;">{value}</p>
                </div>
                ''' for label, value in [
                    ("Pages", total_pages),
                    ("Forms", forms),
...
def dom_summary_card(stats: Dict[str, int]) -> None:
    ...
    st.markdown(
        ...
        <div style="
            ...
            padding: 1.25rem;
        ">
            <h4 style="color: #F1F5F9; margin: 0 0 1rem; font-size: 0.95rem;">DOM Summary</h4>
        """,
        unsafe_allow_html=True,
    )
...
def confidence_indicator(value: int, size: str = "large") -> None:
    ...
    st.markdown(
        ...
        <div style="
            ...
            border-radius: 50%;
            background: conic-gradient({conf_color} {value}%, rgba(51, 65, 85, 0.3) 0%);
            display: flex;
            align-items: center;
            justify-content: center;
            position: relative;
        ">
            <div style="
                ...
                justify-content: center;
            ">
                <span style="color: {conf_color}; font-size: {style['font_size']}; font-weight: 700;">{value}%</span>
            </div>
        </div>
        """,
        unsafe_allow_html=True,
