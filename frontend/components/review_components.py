...
def review_header(
    ...
    st.markdown(f"""<div style=" background: linear-gradient(135deg, rgba(245, 158, 11, 0.15) 0%, rgba(30, 30, 63, 0.95) 100%); border: 1px solid rgba(245, 158, 11, 0.3); border-radius: 16px; padding: 1.5rem; margin-bottom: 1rem; "> <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 1rem;"> <div> <div style="display: flex; align-items: center; gap: 0.5rem; margin-bottom: 0.5rem;"> <span style="color: #64748B; font-size: 0.8rem;">🏠 Dashboard</span> <span style="color: #64748B;">›</span> <span style="color: #64748B; font-size: 0.8rem;">Reviews</span> <span style="color: #64748B;">›</span> <span style="color: #F1F5F9; font-size: 0.8rem;">{mission_name}</span> </div> <div style="display: flex; align-items: center; gap: 1rem;"> <h1 style="margin: 0; font-size: 1.5rem; color: #F1F5F9;">🔍 Human Review Center</h1> <span style=" display: inline-flex; align-items: center; gap: 0.5rem; padding: 0.35rem 0.75rem; border-radius: 9999px; font-size: 0.75rem; background: {status_color}20; color: {status_color}; "> <span style="width: 8px; height: 8px; border-radius: 50%; background: {status_color};"></span> {status_labels.get(status, status)} </span> </div> </div> <div style="display: flex; gap: 0.75rem; flex-wrap: wrap;"> <div style="text-align: center; padding: 0.5rem 1rem; background: rgba(30, 30, 63, 0.8); border-radius: 8px;"> <p style="color: #64748B; margin: 0; font-size: 0.7rem;">Test</p> <p style="color: #F1F5F9; margin: 0; font-size: 0.85rem;">{test_name}</p> </div> <div style="text-align: center; padding: 0.5rem 1rem; background: rgba(30, 30, 63, 0.8); border-radius: 8px;"> <p style="color: #64748B; margin: 0; font-size: 0.7rem;">Agent</p> <p style="color: #6366F1; margin: 0; font-size: 0.85rem;">{agent}</p> </div> <div style="text-align: center; padding: 0.5rem 1rem; background: rgba(30, 30, 63, 0.8); border-radius: 8px;"> <p style="color: #64748B; margin: 0; font-size: 0.7rem;">Confidence</p> <p style="color: #10B981; margin: 0; font-size: 0.85rem;">{confidence}%</p> </div> </div> </div> </div>""", unsafe_allow_html=True)
...
def evidence_panel(
    ...
    st.markdown("""<div style=" background: linear-gradient(135deg, rgba(30, 30, 63, 0.95) 0%, rgba(99, 102, 241, 0.1) 100%); border: 1px solid rgba(99, 102, 241, 0.3); border-radius: 16px; padding: 1.5rem; "> <h4 style="color: #F1F5F9; margin: 0 0 1.25rem; font-size: 1rem;">📋 Evidence Summary</h4>""", unsafe_allow_html=True)
    
    # Expected vs Actual
    col1, col2 = st.columns(2)
    
    with col1:
        st.markdown("""<div style=" padding: 1rem; background: rgba(16, 185, 129, 0.1); border: 1px solid rgba(16, 185, 129, 0.3); border-radius: 12px; "> <p style="color: #10B981; margin: 0 0 0.5rem; font-size: 0.75rem; font-weight: 500;">✓ Expected Result</p> <p style="color: #F1F5F9; margin: 0; font-size: 0.9rem;">""" + expected + """</p> </div>""", unsafe_allow_html=True)
    
    with col2:
        st.markdown("""<div style=" padding: 1rem; background: rgba(99, 102, 241, 0.1); border: 1px solid rgba(99, 102, 241, 0.3); border-radius: 12px; "> <p style="color: #6366F1; margin: 0 0 0.5rem; font-size: 0.75rem; font-weight: 500;">📝 Actual Result</p> <p style="color: #F1F5F9; margin: 0; font-size: 0.9rem;">""" + actual + """</p> </div>""", unsafe_allow_html=True)
    ...
    for assertion in assertions:
        ...
        st.markdown(f"""<div style=" display: flex; align-items: center; justify-content: space-between; padding: 0.75rem 1rem; background: rgba(30, 30, 63, 0.8); border: 1px solid rgba(99, 102, 241, 0.2); border-radius: 8px; margin-bottom: 0.5rem; "> <div style="flex: 1;"> <p style="color: #F1F5F9; margin: 0 0 0.25rem; font-size: 0.85rem;">{assertion['name']}</p> <p style="color: #64748B; margin: 0; font-size: 0.75rem;"> Expected: <span style="color: #10B981;">{assertion['expected']}</span> | Actual: <span style="color: #22D3EE;">{assertion['actual']}</span> </p> </div> <span style="font-size: 1.25rem;">{status_icon}</span> </div>""", unsafe_allow_html=True)
...
def browser_comparison(key_prefix: str = "browser") -> None:
    ...
    for action in actions:
        ...
        st.markdown(f"""<div style=" display: flex; align-items: center; gap: 1rem; padding: 0.75rem 1rem; background: rgba(30, 30, 63, 0.8); border: 1px solid rgba(99, 102, 241, 0.2); border-radius: 8px; margin-bottom: 0.5rem; {'animation: glow 2s infinite;' if is_running else ''} "> <div style=" width: 36px; height: 36px; border-radius: 8px; background: {action['color']}20; display: flex; align-items: center; justify-content: center; font-size: 1.25rem; ">{action['icon']}</div> <div style="flex: 1;"> <p style="color: #F1F5F9; margin: 0; font-size: 0.9rem; font-weight: 500;">{action['name']}</p> <p style="color: #64748B; margin: 0.25rem 0 0; font-size: 0.75rem;">{action['detail']}</p> </div> <span style=" padding: 0.25rem 0.75rem; border-radius: 4px; font-size: 0.75rem; background: {action['color']}20; color: {action['color']}; "> {'⟳ Running' if is_running else '✓' if action['status'] == 'completed' else '○'} </span> </div>""", unsafe_allow_html=True)
    ...
    # Annotation Legend
    st.markdown("""<div style="display: flex; gap: 1rem; margin-top: 1rem; padding: 0.75rem; background: rgba(51, 65, 85, 0.5); border-radius: 8px;"> <div style="display: flex; align-items: center; gap: 0.5rem;"> <div style="width: 12px; height: 12px; border: 2px solid #6366F1; border-radius: 2px;"></div> <span style="color: #94A3B8; font-size: 0.75rem;">AI Focus</span> </div> <div style="display: flex; align-items: center; gap: 0.5rem;"> <div style="width: 12px; height: 12px; border: 2px dashed #10B981; border-radius: 2px;"></div> <span style="color: #94A3B8; font-size: 0.75rem;">Validated</span> </div> <div style="display: flex; align-items: center; gap: 0.5rem;"> <div style="width: 12px; height: 12px; border: 2px solid #EF4444; border-radius: 2px;"></div> <span style="color: #94A3B8; font-size: 0.75rem;">Failed</span> </div> </div>""", unsafe_allow_html=True)
...
def ai_review_panel(ai_review: Dict) -> None:
    ...
    st.markdown(f"""<div style=" background: linear-gradient(135deg, rgba(99, 102, 241, 0.15) 0%, rgba(30, 30, 63, 0.95) 100%); border: 1px solid rgba(99, 102, 241, 0.3); border-radius: 16px; padding: 1.5rem; height: 100%; "> <div style="display: flex; align-items: center; gap: 0.75rem; margin-bottom: 1.25rem;"> <div style=" width: 40px; height: 40px; border-radius: 50%; background: linear-gradient(135deg, #6366F1, #8B5CF6); display: flex; align-items: center; justify-content: center; font-size: 1.25rem; ">🤖</div> <div> <h4 style="color: #F1F5F9; margin: 0; font-size: 1rem;">AI Review</h4> <p style="color: #64748B; margin: 0.25rem 0 0; font-size: 0.75rem;">Automated analysis</p> </div> </div> <!-- Current Observation --> <div style="margin-bottom: 1rem;"> <p style="color: #64748B; margin: 0 0 0.5rem; font-size: 0.75rem;">💭 Current Observation</p> <div style="background: rgba(99, 102, 241, 0.1); border: 1px solid rgba(99, 102, 241, 0.2); border-radius: 8px; padding: 0.75rem;"> <p style="color: #F1F5F9; margin: 0; font-size: 0.85rem;">{ai_review['observation']}</p> </div> </div> <!-- Reasoning --> <div style="margin-bottom: 1rem;"> <p style="color: #64748B; margin: 0 0 0.5rem; font-size: 0.75rem;">🧠 Reasoning</p> <p style="color: #94A3B8; margin: 0; font-size: 0.8rem;">{ai_review['reasoning']}</p> </div> <!-- Evidence --> <div style="margin-bottom: 1rem;"> <p style="color: #64748B; margin: 0 0 0.5rem; font-size: 0.75rem;">📋 Evidence</p> {''.join([f'<p style="color: #10B981; margin: 0.25rem 0; font-size: 0.8rem;">▸ {e}</p>' for e in ai_review['evidence']])} </div> <!-- Confidence --> <div style="margin-bottom: 1rem;"> <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.5rem;"> <span style="color: #64748B; font-size: 0.75rem;">Confidence</span> <span style="color: {conf_color}; font-weight: 600;">{ai_review['confidence']}%</span> </div> <div style="height: 6px; background: #334155; border-radius: 3px; overflow: hidden;"> <div style="width: {ai_review['confidence']}%; height: 100%; background: {conf_color}; border-radius: 3px;"></div> </div> </div> <!-- Suggested Action --> <div style=" background: rgba(16, 185, 129, 0.1); border: 1px solid rgba(16, 185, 129, 0.3); border-radius: 8px; padding: 0.75rem; margin-bottom: 1rem; "> <p style="color: #10B981; margin: 0 0 0.5rem; font-size: 0.75rem; font-weight: 500;">→ Suggested Action</p> <p style="color: #94A3B8; margin: 0; font-size: 0.8rem;">{ai_review['suggested_action']}</p> </div> <!-- Potential Risks --> <div style="margin-bottom: 1rem;"> <p style="color: #F59E0B; margin: 0 0 0.5rem; font-size: 0.75rem; font-weight: 500;">⚠️ Potential Risks</p> {''.join([f'<p style="color: #94A3B8; margin: 0.25rem 0; font-size: 0.8rem;">▸ {r}</p>' for r in ai_review['potential_risks']])} </div> <!-- Best Practices --> <div> <p style="color: #22D3EE; margin: 0 0 0.5rem; font-size: 0.75rem; font-weight: 500;">💡 Best Practices</p> {''.join([f'<p style="color: #94A3B8; margin: 0.25rem 0; font-size: 0.8rem;">▸ {b}</p>' for b in ai_review['best_practices']])} </div> </div>""", unsafe_allow_html=True)
...
def review_timeline() -> None:
    ...
    st.markdown("""<div style=" background: rgba(30, 30, 63, 0.8); border: 1px solid rgba(99, 102, 241, 0.2); border-radius: 16px; padding: 1.5rem; "> <h4 style="color: #F1F5F9; margin: 0 0 1.5rem; font-size: 1rem;">📅 Review Timeline</h4>""", unsafe_allow_html=True)
    ...
    for i, step in enumerate(steps):
        ...
        st.markdown(f"""<div style="display: flex; gap: 1rem; margin-bottom: {0 if is_last else 1}rem;"> <div style="display: flex; flex-direction: column; align-items: center;"> <div style=" width: 28px; height: 28px; border-radius: 50%; background: {color}; display: flex; align-items: center; justify-content: center; color: white; font-size: 0.8rem; ">{icon}</div> {'' if is_last else f'<div style="width: 2px; flex: 1; min-height: 30px; background: {color}; margin-top: 0.5rem;"></div>'} </div> <div style="flex: 1; padding-bottom: {0 if is_last else 0.75}rem;"> <p style="color: {'#F1F5F9' if step['status'] != 'pending' else '#64748B'}; margin: 0 0 0.25rem; font-size: 0.85rem;">{step['name']}</p> <p style="color: #64748B; margin: 0; font-size: 0.7rem;">{step['time']}</p> </div> </div>""", unsafe_allow_html=True)
...
def bug_preview_panel() -> None:
    ...
    st.markdown("""<div style=" background: linear-gradient(135deg, rgba(239, 68, 68, 0.15) 0%, rgba(30, 30, 63, 0.95) 100%); border: 1px solid rgba(239, 68, 68, 0.3); border-radius: 16px; padding: 1.5rem; "> <div style="display: flex; align-items: center; gap: 0.75rem; margin-bottom: 1.25rem;"> <div style=" width: 40px; height: 40px; border-radius: 50%; background: linear-gradient(135deg, #EF4444, #F59E0B); display: flex; align-items: center; justify-content: center; font-size: 1.25rem; ">🐛</div> <div> <h4 style="color: #F1F5F9; margin: 0; font-size: 1rem;">Bug Report Preview</h4> <p style="color: #64748B; margin: 0.25rem 0 0; font-size: 0.75rem;">Generated from failed assertion</p> </div> </div>""", unsafe_allow_html=True)
...
def modification_panel() -> None:
    ...
    st.markdown("""<div style=" background: linear-gradient(135deg, rgba(99, 102, 241, 0.15) 0%, rgba(30, 30, 63, 0.95) 100%); border: 1px solid rgba(99, 102, 241, 0.3); border-radius: 16px; padding: 1.5rem; "> <h4 style="color: #F1F5F9; margin: 0 0 1.25rem; font-size: 1rem;">✏️ Modify Test</h4>""", unsafe_allow_html=True)
...
def review_tabs() -> None:
    ...
    with tabs[1]:
        ...
        browser_comparison(key_prefix="tabs_browser")
...
[WARN] Assertion failed: sidebar.visible
    ...
    with tabs[9]:
        ...
        for col, (label, value, color) in zip([col1, col2, col3, col4], metrics):
            with col:
                st.markdown(f"""<div style="text-align: center; padding: 1rem; background: rgba(30, 30, 63, 0.8); border-radius: 8px;"> <p style="color: #64748B; margin: 0; font-size: 0.75rem;">{label}</p> <p style="color: {color}; margin: 0.5rem 0 0; font-size: 1.5rem; font-weight: 600;">{value}</p> </div>""", unsafe_allow_html=True)
